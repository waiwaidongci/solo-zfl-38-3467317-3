...
- **幂等与乐观锁**：
  ...
  - `Idempotency-Key` 仅在**同一动作作用域**（动作 + 批次，如 `batch.reschedule:B-7`）内回放，
    同键用于不同动作返回 `409 idempotency_scope_conflict`，绝不回放别的动作的响应；
...
21 个用例（`node:test`，无外部测试框架）：

- `tests/api.test.js`（19 项）：越权拒绝、幂等不重复、排期三约束、
